...
class Validation {
     ...
     * @param string $rule
     * @param mixed $value
     * @param compeare-params $params
     * @param invalid statement-message $name
     * @return object $this
     */
    ...
    public function validate() {
        ...
        if (count($this->_rules) > 0) {
            ...
            foreach ($this->_rules as $v) {
                ...
                $valueToCheck = $v['val'];
                $parameters = $v['par'];
                if (!$this->$mehod($valueToCheck, $parameters)) {
                    if ($v['name']) {
                        $this->_errors[] = $v['name'];
                    } else {
                        $this->_errors[] = $v['rule'];
    ...
    public function getErrorsLikeString($multyLine = false) {
        $error = "";
        $multy = "!";
        if ($multyLine === true) {
            $multy = "!<br>";
